...
public class UserMealsUtil {
    public static void main(String[] args) {
        ...
        List<UserMealWithExceed> filteredMeals =  getFilteredWithExceededStream(mealList, LocalTime.of(7, 0), LocalTime.of(12,0), 2000);
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededStream(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> groupedMeals = meals.stream()
                .collect(Collectors.groupingBy(meal -> meal.getDateTime().toLocalDate(), Collectors.summingInt(UserMeal::getCalories)));
        return meals.stream()
                .filter(meal -> isBetweenTimes(meal.getDateTime().toLocalTime(), startTime, endTime))
